...
[![Version](https://img.shields.io/badge/version-1.3.4-emerald.svg)](CHANGELOG.md)
...
| **🦊 Dual-Engine Stealth**  | Arsitektur switchable: Playwright Chromium & Camoufox Anti-Detect Firefox dengan pergerakan mouse kurva Bezier biologis (`humanize: 0.5`).          |
...
| **🎯 Target Workbench**     | Batch engagement (**Like**, **Repost**, **Comment**) berketahanan tinggi dengan selector SVG signature dan distribusi 1-to-1 balasan unik.         |
...
| **🛰️ Live Fleet Telemetry** | Progress bar task global di semua tab, status engine nyata (`OFFLINE` / `BUSY` / `ONLINE`), badge browser engine aktif, dan shortcut (`1-9`, `/`).|
| **🔒 Stealth Defense**      | WebRTC IP leak shield, mitigasi fingerprinting, penundaan human cadence (_jitter typing/scroll_), dan atomic database writes.                      |
