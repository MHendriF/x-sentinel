...
- **Name**: X-SENTINEL v1.3.5
- **Domain**: Autonomous Multi-Node Fleet Cockpit & AI Growth Studio for X (Twitter).
...
- **Automation**: Dual-Engine Architecture (Playwright Chromium & Camoufox Anti-Detect Firefox).
- **Database**: Zero external DB. Atomic JSON Engine (`server/db.js`).
- **Security model**: loopback-only server (127.0.0.1), Origin/Host guard, secret masking (`server/security.js`), path-traversal shield (`server/db.js`).
...
### J. Path Traversal Hardening (`getSafeCommentsFilePath`)

- **Rule**: Never resolve or write files using unsanitized user input filenames. Always use `db.getSafeCommentsFilePath(fileName)` from `server/db.js` which applies `path.basename()`, strips invalid characters, and verifies that the canonical path stays locked inside `data/comments/`.
- **Reason**: Prevents directory traversal attacks (`../../`) from overwriting server files or accessing unauthorized directories.

### K. Modular Cockpit Directory Architecture

- **Rule**: Keep deck surfaces modular. Subcomponents, cards, and modal dialogs must reside in domain subdirectories under `client/src/components/cockpit/` (`about/`, `ai/`, `audit/`, `payloadBank/`, `postStudio/`).
- **Reason**: Preserves single-responsibility principles, prevents multi-thousand-line monolithic components, and keeps testability high.

### L. Anti-AI-Slop & Quotes Cleansing

- **Rule**: Automated replies generated by AI must strip surrounding double quotation marks (`"..."`) and adhere to native conversational brevity. Any response that resembles generic AI slop (_"In today's fast-paced world...", "Here are 5 key takeaways..."_) must be filtered by guardrails.

### M. 280-Character Barometer Meter

- **Rule**: Any post composer or comment editor must provide an interactive 280-character barometer that visually indicates character headroom and warns before reaching X limit thresholds.

### N. Lexical Framework Keystroke Safety (`page.keyboard`)

- **Rule**: Always type through single-focus `page.keyboard.type(char)` in `server/automation/bot/humanCadence.js`. Never invoke repeated `element.type()` on X's Lexical contenteditable fields.
- **Reason**: Repeated element typing causes focus bounce and selection resets, resulting in lost words and corrupt drafts.

### O. Camoufox Persistent Profile Lifecycle

- **Rule**: Manage persistent browser profiles exclusively via `server/automation/bot/camoufoxLoginManager.js`. Deleting or resetting a session profile (`data/camoufox_profiles/<accountId>`) must never alter or wipe out stored account credentials (`auth_token`, `ct0`, proxy configuration) in `data/accounts.json`.

### P. Daily Rotating Logger Discipline

- **Rule**: All server and bot logging must route through `server/logger.js`. Never write custom log files directly to `data/`. The daily logger automatically manages date stamps (`x-sentinel-YYYY-MM-DD.log`) and enforces 30-day retention pruning.

### Q. Rate Limit (344/185) & Challenge Interception

- **Rule**: Engagement and publishing pipelines must inspect GraphQL responses for error codes `344` (Daily limit reached) and `185` (Status update limit), as well as DOM modal challenges ("Add a phone / Daily limit"). Tasks must abort or pause gracefully upon detection to prevent account locking.
