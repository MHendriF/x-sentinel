      ...
      <div className="flex w-full max-w-md flex-col gap-4 rounded-xl border border-amber-500/40 bg-obsidian-850 p-5 shadow-2xl">
        ...
        <div className="flex flex-col gap-3">
          <div className="space-y-1">
            <label className="font-mono text-xs text-slate-300">SCHEDULE TITLE (LABEL)</label>
            <Input
              type="text"
              value={scheduleTitle}
              onChange={(e) => setScheduleTitle(e.target.value)}
              placeholder="e.g. Crypto Morning Insight #1"
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1">
              <label className="font-mono text-xs text-slate-300">EXECUTION DATE</label>
              <Input
                type="date"
                value={scheduleDate}
            ...
            <div className="space-y-1">
              <label className="font-mono text-xs text-slate-300">TIME (HH:MM)</label>
              <Input
                type="time"
                value={scheduleTime}
          ...
          <div className="space-y-1">
            <label className="font-mono text-xs text-slate-300">NODE ROTATION DELAY (SECONDS)</label>
            <Input
              type="number"
              value={scheduleDelay}
          ...
          <div className="rounded-lg border border-border/80 bg-obsidian-900/60 p-3 font-mono text-[11px] text-slate-400">
            <div className="flex justify-between">
              ...
              <span className="font-bold text-white">
                {targetMode === 'single'
                  ? selectedAccountName || '1 Account'
                  : `Entire Fleet (${activeAccountsCount} Nodes)`}
              </span>
            </div>
            <div className="mt-1 flex justify-between">
              <span>Scheduler Engine:</span>
              <span className="text-emerald">Background Loop Active (15s)</span>
            </div>
          </div>
        </div>
